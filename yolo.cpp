...
bool Yolo::Detect(cv::Mat& SrcImg, Net& net, vector<Output>& output) {
   cv:: Mat blob;
    int col = SrcImg.cols;
    int row = SrcImg.rows;
   ...
   cv:: Mat netInputImg = SrcImg.clone();
    ...
    ratio_h = (float)netInputImg.rows / netHeight;
    ratio_w = (float)netInputImg.cols / netWidth;
                ...
                for (int j = 0; j < grid_y; j++) {
                    ...
                    if (box_score > boxThreshold) {
                        ...
                        if (max_class_socre > classThreshold) {
                            //rect [x,y,w,h]
                            x = pdata[0];//float x = (Sigmoid(pdata[0]) * 2.f - 0.5f + j) * netStride[stride];  //x
                            y = pdata[1];//float y = (Sigmoid(pdata[1]) * 2.f - 0.5f + i) * netStride[stride];   //y
                            w = pdata[2];//float w = powf(Sigmoid(pdata[2]) * 2.f, 2.f) * anchor_w;   //w
                            h = pdata[3];//float h = powf(Sigmoid(pdata[3]) * 2.f, 2.f) * anchor_h;  //h
                            int left = (x - 0.5 * w) * ratio_w;
                            int top = (y - 0.5 * h) * ratio_h;
                            cx= (int)(x*ratio_w);
                            cy =(int)(y*ratio_h);

                            sprintf(temp_1, "(x:%d,y:%d)", cx, cy);
                            putText(SrcImg, temp_1, Point(cx, cy), FONT_HERSHEY_COMPLEX, 1, Scalar(0, 0, 255),1,LINE_AA);
                            circle(SrcImg, Point(cx, cy), 3, Scalar(0, 0, 255),FILLED);
...
void Yolo::drawPred(cv::Mat& img, vector<Output> result, vector<Scalar> color) {
    for (int i = 0; i < result.size(); i++) {
        ...
        //int center_x =x;
        //int center_y = ;
        //sprintf(temp_2, "(x:%d,y:%d)", left, top);
        //putText(img, temp_2, Point(left, top), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 255, 255));
        //circle(img, Point(left, top), 3, Scalar(255, 0, 0), 1);
    }
